...
export default function Radiate() {
  ...
  const handleDrag = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (
        ...
        // Add the new word to the "uncollected" state so it appears at the cursor
        setUncollectedQuoteWords(prev => [...prev, newQuoteWord]);

        // After a delay, move the word from uncollected to collected
        setTimeout(() => {
  ...
  return (
    <LayoutGroup>
        <div
            ...
            className="flex items-center justify-center h-screen w-full bg-gradient-to-br from-[var(--color-joy-start)] to-[var(--color-joy-end)] text-[var(--color-joy-text)] cursor-crosshair overflow-hidden relative"
        >
            ...
            <AnimatePresence>
                {sparkles.map(sparkle => (
                    <motion.div
                        key={sparkle.id}
                        initial={{ opacity: 0, x: `${sparkle.x}vw`, y: `${sparkle.y}vh` }}
                        animate={{ opacity: [0, 1, 0], y: `${sparkle.y - 10}vh`}}
                        transition={{ duration: sparkle.duration, ease: "linear" }}
                        style={{ position: 'absolute', width: `${sparkle.size}px`, height: `${sparkle.size}px`, backgroundColor: 'white', borderRadius: '50%', boxShadow: '0 0 8px 2px rgba(255, 255, 255, 0.7)'}}
                    />
                ))}
            </AnimatePresence>
                ...
                <AnimatePresence>
                    {rays.map((ray) => (
                        <motion.div
                            ...
                            style={{ transformOrigin: "left center", rotate: ray.rotation, top: "50%", left: "50%" }}
                        />
                    ))}
                </AnimatePresence>
            ...
            <AnimatePresence>
             ...
             {/* Render uncollected quote words at their iniftial position */}
